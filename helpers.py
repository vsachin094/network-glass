...
def get_vars(router_name, cmd, ipprefix):
  ...
  switcher = {
    'bgp': {
      ...
      'IOS-XE': 'show bgp ipv4 unicast {}'.format(ipprefix),
      # 'NXOS': 'show bgp ipv4 unicast {}'.format(ipprefix),
      'NXOS': 'show bgp ipv4 unicast detail'
    },
    'traceroute': {
      'JunOS': 'traceroute {} wait 1'.format(ipprefix),
      'IOS-XR': 'traceroute {} timeout 1 probe 2'.format(ipprefix),
      'IOS-XE': 'traceroute {} timeout 1 probe 2'.format(ipprefix),
      'NXOS': 'traceroute {} timeout 1 probe 2'.format(ipprefix),
    },
    'ping': {
      'JunOS': 'ping {} count 5'.format(ipprefix),
      'IOS-XR': 'ping {} count 5'.format(ipprefix),
      'IOS-XE': 'ping {} count 5'.format(ipprefix),
      'NXOS': 'ping {} count 5'.format(ipprefix)
    }
  }
